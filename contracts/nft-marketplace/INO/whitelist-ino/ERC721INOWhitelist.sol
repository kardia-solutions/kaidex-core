...
contract ERC721INOWhitelist is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(IMinterAdapterWhitelist(_minter).isMinter(), "minter is invalid");
        buyToken = _buyToken;
    ...
    function claims(uint256 _ticket) public whenNotPaused nonReentrant satisfyClaimCondition {
        ...
        if (_ticket > 0) {
            for(uint256 index = 0; index < _ticket; ++index) {
                uint256 tokenId_ = IMinterAdapterWhitelist(minterAdapter).mint(_msgSender());
    ...
    function setMinter(address _newMinter) public onlyOwner {
        require(IMinterAdapterWhitelist(_newMinter).isMinter(), "minter is invalid");
    ...
    function getMaximumTicketByUser (address userAddr) public view returns (uint256) {
        return IMinterAdapterWhitelist(minterAdapter).maximumTicketByUser(userAddr);
    }

    function getMaximumTicketSales () public view returns (uint256) {
        return IMinterAdapterWhitelist(minterAdapter).maximumNFTSales();
    }
