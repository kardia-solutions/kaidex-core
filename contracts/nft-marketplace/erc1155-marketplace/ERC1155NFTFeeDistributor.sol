...
contract ERC1155NFTFeeDistributor is Ownable {
    ...
    function setProtocolFeeRecipient(address _recipient) external onlyOwner {
        require(
            ...
            "ERC1155NFTFeeDistributor: zero address"
        );
        protocolFeeRecipient = _recipient;
    }
    ...
    function _distributeFees(
        ...
        if (protocolFeeRecipient != address(0)) {
            fee = _totalPay.mul(protocolFeePercent).div(10000);
        ...
        if (cfg.loyaltyFeeRecipient != address(0)) {
            fee = _totalPay.mul(cfg.loyaltyFeePercent).div(10000);
            if (fee > 0) {
