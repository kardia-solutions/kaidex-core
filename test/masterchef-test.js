...
describe("MasterChef", function () {
    ...
    it("should set correct state variables", async function () {
        this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "0", this.bar.address)
        await this.chef.deployed()
        await this.kdx.transferOwnership(this.chef.address)
        const kdx = await this.chef.kdx()
        const bar = await this.chef.bar()
        const owner = await this.kdx.owner()
        expect(kdx).to.equal(this.kdx.address)
        expect(bar).to.equal(this.bar.address)
        expect(owner).to.equal(this.chef.address)
    })

    it("should allow operator and only operator to update bar", async function () {
        this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "0", this.bar.address)
        await this.chef.deployed()
        await expect(this.chef.connect(this.bob).setNewBar(this.bob.address, { from: this.bob.address })).to.be.revertedWith("Ownable: caller is not the owner")
        await this.chef.setNewBar(this.bob.address)
        expect(await this.chef.bar()).to.equal(this.bob.address)
        await this.chef.setNewBar(this.alice.address)
        expect(await this.chef.bar()).to.equal(this.alice.address)
    })
    ...
    context("With ERC/LP token added to the field", function () {
        ...
        it("should allow emergency withdraw", async function () {
            ...
            this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "100", this.bar.address)
            await this.chef.deployed()
            await this.chef.add("100", this.lp.address, "0x0000000000000000000000000000000000000000", true)
        ...
        it("should give out KDXs only after farming time", async function () {
            ...
            await this.chef.deployed()
            await this.kdx.transferOwnership(this.chef.address)
            await this.chef.add("100", this.lp.address, "0x0000000000000000000000000000000000000000", true)  // add pool
            ...
            // expect(await this.kdx.balanceOf(this.bob.address)).to.equal("5000")
            // expect(await this.kdx.balanceOf(this.dev.address)).to.equal("500")
            // expect(await this.kdx.totalSupply()).to.equal("5500")
        })

        // it("should not distribute KDXs if no one deposit", async function () {
        //     // 100 per block farming rate starting at block 200 with bonus until block 1000
        //     this.chef = await this.MasterChef.deploy(this.kdx.address, this.dev.address, "100", "200", "1000")
        //     await this.chef.deployed()
        //     await this.kdx.transferOwnership(this.chef.address)
        ...
        //     expect(await this.kdx.totalSupply()).to.equal("0")
        //     expect(await this.kdx.balanceOf(this.bob.address)).to.equal("0")
        //     expect(await this.kdx.balanceOf(this.dev.address)).to.equal("0")
        ...
        //     expect(await this.kdx.totalSupply()).to.equal("11000")
        //     expect(await this.kdx.balanceOf(this.bob.address)).to.equal("10000")
        //     expect(await this.kdx.balanceOf(this.dev.address)).to.equal("1000")
        //     expect(await this.lp.balanceOf(this.bob.address)).to.equal("1000")
        // })

        // it("should distribute KDXs properly for each staker", async function () {
        //     // 100 per block farming rate starting at block 300 with bonus until block 1000
        //     this.chef = await this.MasterChef.deploy(this.kdx.address, this.dev.address, "100", "300", "1000")
        //     await this.chef.deployed()
        //     await this.kdx.transferOwnership(this.chef.address)
        //     await this.chef.add("100", this.lp.address, true)
        ...
        //     await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address })
        //     expect(await this.kdx.totalSupply()).to.equal("11000")
        //     expect(await this.kdx.balanceOf(this.alice.address)).to.equal("5666")
        //     expect(await this.kdx.balanceOf(this.bob.address)).to.equal("0")
        //     expect(await this.kdx.balanceOf(this.carol.address)).to.equal("0")
        //     expect(await this.kdx.balanceOf(this.chef.address)).to.equal("4334")
        //     expect(await this.kdx.balanceOf(this.dev.address)).to.equal("1000")
        //     // Bob withdraws 5 LPs at block 330. At this point:
        //     //   Bob should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
        //     await advanceBlockTo("329")
        //     await this.chef.connect(this.bob).withdraw(0, "5", { from: this.bob.address })
        //     expect(await this.kdx.totalSupply()).to.equal("22000")
        //     expect(await this.kdx.balanceOf(this.alice.address)).to.equal("5666")
        //     expect(await this.kdx.balanceOf(this.bob.address)).to.equal("6190")
        //     expect(await this.kdx.balanceOf(this.carol.address)).to.equal("0")
        //     expect(await this.kdx.balanceOf(this.chef.address)).to.equal("8144")
        //     expect(await this.kdx.balanceOf(this.dev.address)).to.equal("2000")
        ...
        //     expect(await this.kdx.totalSupply()).to.equal("55000")
        //     expect(await this.kdx.balanceOf(this.dev.address)).to.equal("5000")
        //     // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
        //     expect(await this.kdx.balanceOf(this.alice.address)).to.equal("11600")
        //     // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
        //     expect(await this.kdx.balanceOf(this.bob.address)).to.equal("11831")
        //     // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
        //     expect(await this.kdx.balanceOf(this.carol.address)).to.equal("26568")
        ...
        //     this.chef = await this.MasterChef.deploy(this.kdx.address, this.dev.address, "100", "400", "1000")
        //     await this.kdx.transferOwnership(this.chef.address)
        ...
        //     expect(await this.chef.pendingKDX(0, this.alice.address)).to.equal("10000")
        //     // Bob deposits 10 LP2s at block 425
        //     await advanceBlockTo("424")
        //     await this.chef.connect(this.bob).deposit(1, "5", { from: this.bob.address })
        //     // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
        //     expect(await this.chef.pendingKDX(0, this.alice.address)).to.equal("11666")
        //     await advanceBlockTo("430")
        //     // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
        //     expect(await this.chef.pendingKDX(0, this.alice.address)).to.equal("13333")
        //     expect(await this.chef.pendingKDX(1, this.bob.address)).to.equal("3333")
        // })


        // it("should stop giving bonus KDXs after the bonus period ends", async function () {
        //     // 100 per block farming rate starting at block 500 with bonus until block 600
        //     this.chef = await this.MasterChef.deploy(this.kdx.address, this.dev.address, "100", "500", "600")
        //     await this.kdx.transferOwnership(this.chef.address)
        //     await this.lp.connect(this.alice).approve(this.chef.address, "1000", { from: this.alice.address })
        ...
        //     expect(await this.chef.pendingKDX(0, this.alice.address)).to.equal("10500")
        ...
        //     expect(await this.kdx.balanceOf(this.alice.address)).to.equal("10600")
        // })
