...
describe("MasterChef", function () {
    ...
    context("With ERC/LP token added to the field", function () {
        ...
        it("should give out KDXs only after farming time", async function () {
            ...
            expect(await this.kdx.balanceOf(this.bar.address)).to.equal("10")
            expect(await this.kdx.totalSupply()).to.equal("510")
        })

        it("should not distribute KDXs if no one deposit", async function () {
            // 100 per block farming rate starting at block 200
            this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "200", this.bar.address)
            await this.chef.deployed()
            await this.kdx.transferOwnership(this.chef.address)
            await this.chef.add("100", this.lp.address, "0x0000000000000000000000000000000000000000", true)  // add pool
            await this.lp.connect(this.bob).approve(this.chef.address, "1000")
            ...
            expect(await this.kdx.totalSupply()).to.equal("0")
            expect(await this.kdx.balanceOf(this.bob.address)).to.equal("0")
            expect(await this.kdx.balanceOf(this.bar.address)).to.equal("0")
            ...
            expect(await this.kdx.totalSupply()).to.equal("1020")
            expect(await this.kdx.balanceOf(this.bob.address)).to.equal("1000")
            expect(await this.kdx.balanceOf(this.bar.address)).to.equal("20")
            expect(await this.lp.balanceOf(this.bob.address)).to.equal("1000")
        })

        it("should distribute KDXs properly for each staker", async function () {
            // 100 per block farming rate starting at block 300
            this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "300", this.bar.address)
            await this.chef.deployed()
            await this.kdx.transferOwnership(this.chef.address)
            await this.chef.add("100", this.lp.address, "0x0000000000000000000000000000000000000000", true)  // add pool
            ...
            expect(await this.kdx.totalSupply()).to.equal("2040")
            expect(await this.kdx.balanceOf(this.alice.address)).to.equal("566")
            ...
            expect(await this.kdx.balanceOf(this.bar.address)).to.equal("40")
            ...
            expect(await this.kdx.totalSupply()).to.equal("5100")
            expect(await this.kdx.balanceOf(this.bar.address)).to.equal("100")
        ...
        it("should give proper KDXs allocation to each pool", async function () {
            ...
            this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "400", this.bar.address)
            await this.kdx.transferOwnership(this.chef.address)
            ...
            await this.chef.add("10", this.lp.address, "0x0000000000000000000000000000000000000000", true)  // add pool
            // Alice deposits 10 LPs at block 410
            await advanceBlockTo("409")
            await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address })
            // Add LP2 to the pool with allocation 2 at block 420
            await advanceBlockTo("419")
            await this.chef.add("20", this.lp2.address, "0x0000000000000000000000000000000000000000", true)
        ...
        it("should stop giving bonus KDXs after the bonus period ends", async function () {
            ...
            this.chef = await this.MasterChef.deploy(this.kdx.address, "100", "500", this.bar.address)
            await this.kdx.transferOwnership(this.chef.address)
            await this.lp.connect(this.alice).approve(this.chef.address, "1000", { from: this.alice.address })
            await this.chef.add("1", this.lp.address, "0x0000000000000000000000000000000000000000", true)
