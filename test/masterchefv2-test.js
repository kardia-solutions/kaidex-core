const { ADDRESS_ZERO, advanceBlock, advanceBlockTo, deploy, getBigNumber, prepare } = require("./utilities");
const { expect, assert } = require("chai");

describe("KaidexMasterChefV2", function () {
  before(async function () {
    await prepare(this, ["KaidexMasterChef", "KaiDexToken", "ERC20Mock", "KaidexMasterChefV2", "RewarderMock", "RewarderBrokenMock"])
    await deploy(this, [["brokenRewarder", this.RewarderBrokenMock]])
  })

  beforeEach(async function () {
    await deploy(this, [["kdx", this.KaiDexToken]])

    await deploy(this, [
      ["lp", this.ERC20Mock, ["LP Token", "LPT", getBigNumber(10)]],
      ["dummy", this.ERC20Mock, ["Dummy", "DummyT", getBigNumber(10)]],
      ["chef", this.KaidexMasterChef, [this.kdx.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.KaidexMasterChefV2, [this.chef.address, this.kdx.address, 1]],
  ...
  describe("PendingKdx", function () {
    it("PendingKdx should equal ExpectedKdx", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedKdx = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingKdx = await this.chef2.pendingKDX(0, this.alice.address)
      expect(pendingKdx).to.be.equal(expectedKdx)
    })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accKdxPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of KDX and reward", async function () {
      ...
      let expectedKdx = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedKdx)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.kdx.balanceOf(this.alice.address))
        .to.be.equal(await this.r.balanceOf(this.alice.address))
        .to.be.equal(expectedKdx)
    })
    ...
    it("Harvest for KDX-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedKdx)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.kdx.balanceOf(this.alice.address)).to.be.equal(expectedKdx)
    })
  })
